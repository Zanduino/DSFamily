...
low temperature alarms; but these are used for calibration purposes by this library. In order to prevent errors when
one or both bytes are modified, the exclusive OR of the two values must be 0xFF in order for a calibration offset to
be applied.
...
void loop()
{
  ...
  for(uint8_t i=0;i<DSFamily.ThermometersFound;i++) // loop for each thermometer found
  {
     ...
     sprintf(buffer,"%02d %02X%02X%02X%02X%02X%02X%02X%02X %04d %3d %04d ",i,ROMBuffer[0],ROMBuffer[1],ROMBuffer[2],
             ROMBuffer[3],ROMBuffer[4],ROMBuffer[5],ROMBuffer[6],ROMBuffer[7],DSFamily.ReadDeviceTemp(i,true),
             DSFamily.GetDeviceCalibration(i),DSFamily.ReadDeviceTemp(i));
     Serial.print(buffer);
     Serial.println(DSFamily.ReadDeviceTemp(i)*DS_DEGREES,3); // Arduino sprintf() doesn't do floating
